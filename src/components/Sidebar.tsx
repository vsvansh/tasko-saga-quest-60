...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return <>
      ...
      <Button variant="ghost" size="icon" className="fixed bottom-4 left-4 z-50 hidden lg:flex" onClick={toggleSidebar}>
      ...
      <AnimatePresence>
        ...
        damping: 20
            ...
            <div className="p-4 border-b border-border flex justify-between items-center">
              ...
              {isMobile && <Button variant="ghost" size="sm" onClick={toggleSidebar}>
                  <XCircle size={18} />
                </Button>}
            </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="px-2 py-4">
                <div className="space-y-1 mb-2">
                  <Button variant={state.filter === 'all' ? "secondary" : "ghost"} className="w-full justify-start font-medium" onClick={() => {
                setFilter('all');
                setSelectedCategory(null);
                  ...
                  <Button variant={state.filter === 'active' ? "secondary" : "ghost"} className="w-full justify-start font-medium" onClick={() => {
                setFilter('active');
                setSelectedCategory(null);
              }}>
                  ...
                  <Button variant={state.filter === 'completed' ? "secondary" : "ghost"} className="w-full justify-start font-medium" onClick={() => {
                setFilter('completed');
                setSelectedCategory(null);
              }}>
                ...
                <div className="space-y-1 mb-2">
                  <Button variant="ghost" className="w-full justify-start font-medium" onClick={handleStarredClick}>
                  ...
                  <Button variant="ghost" className="w-full justify-start font-medium" onClick={handleNotificationsClick}>
              ...
              <ScrollArea className="flex-1 px-2 pb-2">
                <DragDropContext onDragEnd={handleDragEnd}>
                  <Droppable droppableId="categories">
                    {provided => <div ref={provided.innerRef} {...provided.droppableProps} className="space-y-1">
                        {state.categories.map((category, index) => <Draggable key={category.id} draggableId={category.id} index={index}>
                            {provided => <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className="relative" onMouseEnter={() => setIsHovering(category.id)} onMouseLeave={() => setIsHovering(null)}>
                                <Button variant={state.selectedCategory === category.id ? "secondary" : "ghost"} className="w-full justify-start" onClick={() => {
                        ...
                        setFilter('all');
                                ...
                                {isHovering === category.id && <div className="absolute right-2 top-1.5 flex items-center space-x-1">
            ...
            <div className="p-4 border-t border-border">
              <Button variant="outline" className="w-full flex items-center gap-2" onClick={onAddCategory}>
                <Plus size={16} />
                Add Category
              </Button>
              <div className="flex gap-2 mt-2">
                <Button variant="ghost" size="sm" className="flex-1 flex items-center gap-2 text-muted-foreground" onClick={handleSettingsClick}>
                  <Settings size={14} />
                  Settings
                </Button>
      ...
      <Dialog open={showStarredTasks} onOpenChange={setShowStarredTasks}>
        <DialogContent className="max-w-3xl">
          ...
          <div className="py-4">
            {state.tasks.filter(task => task.starred).length > 0 ? <div className="space-y-2">
                {state.tasks.filter(task => task.starred).map(task => <div key={task.id} className="p-3 border rounded-md">
                    <div className="flex items-center justify-between">
                      <div>
      ...
      <Dialog open={showNotifications} onOpenChange={setShowNotifications}>
        <DialogContent>
          ...
          <div className="py-4">
            {dueSoonTasks > 0 ? <div className="space-y-2">
              ...
              const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
              return diffDays <= 3 && diffDays >= 0 && !task.completed;
            }).map(task => <div key={task.id} className="p-3 border rounded-md">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-medium">{task.title}</h4>
                        {task.description && <p className="text-sm text-muted-foreground">{task.description}</p>}
